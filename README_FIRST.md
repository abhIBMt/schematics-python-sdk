...
that will guide you in the required modifications:
  ...
  - `.github/workflows/build.yaml`:
    - In the `.github/workflows` directory remove the `build.yaml` file, then rename `build.yaml.sdk` to `build.yaml`.

  - `.github/workflows/publish.yaml`:
    - In the `.github/workflows` rename `publis.yaml.sdk` to `publish.yaml`.
...
This repository is set up to use [GitHub Actions](https://docs.github.com/en/actions)
for continuous integration.

The `.github/workflows/build.yaml` file contains all the instructions necessary to run the build.
An example `build.yaml` file is supplied with this template repository.

For details related to the GitHub workflow file, see the documentation of
[GitHub Actions](https://docs.github.com/en/actions)

## Release management with semantic-release
The `build.yaml` file included in this template repository is configured to
